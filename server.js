  ...
  try {
    ...
    if (update.message) {
      ...
      if (text) {
        ...
        const languageCode = message.from.language_code || 'ru';
        
        // Тексты на двух языках
        const welcomeText = languageCode === 'kk' ? 
          `🧁 Қош келдіңіз, ${firstName}!

Бұл бот не істей алады?
Mini App ашып, жаңа пісірілген өнімдерді есігіңізге жеткізуге тапсырыс беріңіз!

🎂 Біздің кондитерлік — қолмен жасалған торттар мен десерттер

Ассортимент:
🍰 Тапсырыс бойынша торттар
🧁 Капкейктер мен маффиндер
🥐 Круассандар мен бөліктер
🍪 Печенье мен пирожныелер
☕ Шай мен кофеге десерттер
🎁 Сыйлық топтамалары
...
Что умеет этот бот?
Откройте наш Mini App и выбирайте свежую выпечку с доставкой прямо к вашей двери!

🎂 Наша Кондитерская — торты и десерты ручной работы
...
⭐ Скидка 10% на первый заказ!`;
        ...
        const buttonText = languageCode === 'kk' ? '🛍️ Дүкенді ашу' : '🛍️ Открыть магазин';
        
        // Отправляем фото с текстом и кнопкой
        await axios.post(`https://api.telegram.org/bot${BOT_TOKEN}/sendPhoto`, {
          chat_id: chatId,
          photo: 'https://yepbabnubbjzvdjnlfzw.supabase.co/storage/v1/object/public/LOGO/variety-of-baked-goods-a-sweet-indulgence-generated-by-ai-free-photo@0.5x.webp',
          caption: welcomeText,
          reply_markup: {
            inline_keyboard: [
              [{ text: buttonText, web_app: { url: CLIENT_APP_URL } }]
            ]
          }
        });
        ...
        await axios.post(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
          chat_id: chatId,
          text: '👇 Используйте кнопки ниже для быстрого доступа:',
          reply_markup: keyboard
        });
